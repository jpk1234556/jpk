...
class AdminReportsView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        if report_type == 'revenue':
            data = self.generate_revenue_report(start_date, end_date, property_id)
        elif report_type == 'occupancy':
            data = self.generate_occupancy_report(start_date, end_date, property_id)
        elif report_type == 'maintenance':
            data = self.generate_maintenance_report(start_date, end_date, property_id)
        elif report_type == 'tenants':
            data = self.generate_tenant_report(start_date, end_date, property_id)
        else:
    ...
    def export_to_csv(self, data, report_type):
        ...
        if report_type == 'revenue':
            ...
            writer.writerow(['Total Revenue (30 days)', data.get('total_revenue_30_days', 0)])
            writer.writerow(['Total Maintenance Requests', data.get('total_maintenance_requests', 0)])
            writer.writerow(['Pending Maintenance Requests', data.get('pending_maintenance_requests', 0)])
